...
import {
  ...
  lift, sobrerrepresentados, resumenLugar, UBIC_DEFECTO,
} from './mapa.js'
...
test('agregación: procedencias por defecto y huecos contados', () => {
  const { porLugar, cuenta } = agrega(ARCHIVE.albums, geo)
  assert.deepEqual([...porLugar.keys()].sort(), [0, 1, 2])
  assert.deepEqual(porLugar.get(0).map((a) => a.id), [1, 2])
  assert.equal(cuenta.localizadas, 5)
  assert.equal(cuenta.tag_hint, 1, 'la pista de tag no se sitúa por defecto, pero se cuenta')
  ...
  assert.equal(cuenta.sinMunicipio, 4)
  assert.equal(cuenta.localizadas + cuenta.sinMunicipio, ARCHIVE.albums.length)
})

test('agregación: activar pistas de tag, quitar inferidas, territorio', () => {
  let r = agrega(ARCHIVE.albums, geo, { ubic: 'dcat' })
  assert.equal(r.porLugar.get(0).length, 3)
  r = agrega(ARCHIVE.albums, geo, { ubic: 'd' })
  assert.equal(r.cuenta.ocultasProcedencia, 2, 'same_account y artist_inferred ocultas y contadas')
  r = agrega(ARCHIVE.albums, geo, { territorio: 'Gipuzkoa' })
  assert.deepEqual([...r.porLugar.keys()], [1])
  assert.equal(r.cuenta.otroTerritorio, 3)
})
...
test('filtros combinados: tag + años + territorio', () => {
  ...
  assert.deepEqual(rank, [['bilbo', 2], ['zarautz', 1]])
